 ...
 */
HWTEST_F(PermissionRecordManagerTest, StartRemoteUsingPermissionTest001, TestSize.Level0)
{
    ...
    int32_t callerPid = CALLER_PID;

    info.permissionName = "";
    EXPECT_EQ(PrivacyError::ERR_PARAM_INVALID, PermissionRecordManager::GetInstance().StartRemoteUsingPermission(
        info, callerPid));
    info.permissionName = "ohos.permission.CAMERA";
    ...
    remoteDeviceId = "";
    EXPECT_EQ(PrivacyError::ERR_PARAM_INVALID, PermissionRecordManager::GetInstance().StopRemoteUsingPermission(
        remoteDeviceId, remoteDeviceName, permissionName, callerPid));
    remoteDeviceId = "ididid";

    remoteDeviceName = "";
    EXPECT_EQ(PrivacyError::ERR_PARAM_INVALID, PermissionRecordManager::GetInstance().StopRemoteUsingPermission(
        remoteDeviceId, remoteDeviceName, permissionName, callerPid));
    remoteDeviceName = "namename";

    permissionName = "";
    EXPECT_EQ(PrivacyError::ERR_PERMISSION_NOT_EXIST, PermissionRecordManager::GetInstance().StopRemoteUsingPermission(
        remoteDeviceId, remoteDeviceName, permissionName, callerPid));
    permissionName = "ohos.permission.CAMERA";
}

/*
 * @tc.name: StartRemoteUsingPermissionTest002
 * @tc.desc: Test edm disallow
 * @tc.type: FUNC
 * @tc.require: issues3049
 */
HWTEST_F(PermissionRecordManagerTest, StartRemoteUsingPermissionTest002, TestSize.Level0)
{
    char value[VALUE_MAX_LEN] = {0};
    GetParameter(EDM_MIC_MUTE_KEY, "", value, VALUE_MAX_LEN - 1);
    GTEST_LOG_(INFO) << "value:" << value;
    ...
    EXPECT_EQ(PrivacyError::ERR_EDM_POLICY_CHECK_FAILED,
        PermissionRecordManager::GetInstance().StartRemoteUsingPermission(info, CALLER_PID));
 ...
 * @tc.desc: PermissionRecordManager::GetPermissionUsedTypeInfos function test
 * @tc.type: FUNC
 * @tc.require:
 */
