...
bool AudioManagerPrivacyProxy::IsMicrophoneMute()
{
    ...
    int32_t error = Remote()->SendRequest(AudioStandard::AudioPolicyCommand::IS_MICROPHONE_MUTE, data, reply, option);
    if (error != ERR_NONE) {
...
int32_t AudioManagerPrivacyProxy::SetMicrophoneMute(bool isMute)
{
    ...
    int32_t error = Remote()->SendRequest(AudioStandard::AudioPolicyCommand::SET_MICROPHONE_MUTE, data, reply, option);
    if (error != ERR_NONE) {
