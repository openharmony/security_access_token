    ...
    bool updateFlag)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "PermFilterList size: %{public}zu", permFilterList.size());
    for (const auto& perm : permFilterList) {
        ...
        if (!PermissionDefinitionCache::GetInstance().HasDefinition(perm.permissionName)) {
            ...
            PermissionDefinitionCache::GetInstance().Update(perm, tokenId);
            LOGI(AT_DOMAIN, AT_TAG, "Permission %{public}s has define", perm.permissionName.c_str());
        }
    }
}
...
{
    LOGI(AT_DOMAIN, AT_TAG, "tokenID: %{public}u", tokenID);
    ...
    AccessTokenID tokenID, const std::string& permissionName)
{
    if ((tokenID == INVALID_TOKENID) ||
        ...
        LOGE(AT_DOMAIN, AT_TAG, "Id: %{public}d is invalid.", tokenID);
...
{
    LOGD(AT_DOMAIN, AT_TAG, "%{public}s called, Id: %{public}u, isSystemGrant: %{public}d",
    ...
    if (infoPtr == nullptr) {
        LOGE(AT_DOMAIN, AT_TAG, "Id %{public}u is invalid.", tokenID);
    ...
    if (ret != RET_SUCCESS) {
        LOGE(AT_DOMAIN, AT_TAG, "GetPermissionStateList failed, Id %{public}u is invalid.", tokenID);
        return ret;
    }
    ...
    const std::string &permission, int32_t& status, uint32_t& flag)
{
    if (!PermissionDefinitionCache::GetInstance().HasHapPermissionDefinitionForHap(permission)) {
        LOGE(AT_DOMAIN, AT_TAG, "No definition for hap permission: %{public}s!", permission.c_str());
        return false;
    }
    ...
    if (iter == permsList.end()) {
        LOGW(AT_DOMAIN, AT_TAG, "Can not find permission: %{public}s define!", permission.c_str());
        return false;
    }
    LOGD(AT_DOMAIN, AT_TAG, "Find goal permission: %{public}s, status: %{public}d, flag: %{public}d",
...
static bool IsPermissionRestrictedByRules(const std::string& permission)
{
    ...
    if (iterator != g_notDisplayedPerms.end()) {
        LOGW(AT_DOMAIN, AT_TAG,
            "Permission is not available to common apps: %{public}s!", permission.c_str());
        return true;
    }
    ...
    PermissionListState& permState, int32_t apiVersion)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "%{public}s: status: %{public}d, flag: %{public}d",
...
{
    LOGI(AT_DOMAIN, AT_TAG, "%{public}s called, Id: %{public}u, permissionName: %{public}s",
        __func__, tokenID, permissionName.c_str());
    if (!PermissionValidator::IsPermissionNameValid(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "PermissionName:%{public}s invalid!", permissionName.c_str());
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    ...
    int32_t userID)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "UserID=%{public}u, permissionName=%{public}s, status=%{public}d", userID,
        permissionName.c_str(), status);
    if (!PermissionValidator::IsUserIdValid(userID)) {
        LOGE(AT_DOMAIN, AT_TAG, "UserID is invalid.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    if (!PermissionValidator::IsPermissionNameValid(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "Permission name is invalid.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    if (!PermissionDefinitionCache::GetInstance().HasDefinition(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "Permission=%{public}s is not defined.", permissionName.c_str());
        return AccessTokenError::ERR_PERMISSION_NOT_EXIST;
    }
    if (PermissionDefinitionCache::GetInstance().IsSystemGrantedPermission(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "Only support permissions of user_grant to set.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    if (!PermissionValidator::IsToggleStatusValid(status)) {
        LOGE(AT_DOMAIN, AT_TAG, "Status is invalid.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    ...
    int32_t userID)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "User=%{public}u, perm=%{public}s", userID, permissionName.c_str());
    if (!PermissionValidator::IsUserIdValid(userID)) {
        LOGE(AT_DOMAIN, AT_TAG, "UserID is invalid.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    ...
    if (PermissionDefinitionCache::GetInstance().IsSystemGrantedPermission(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "Only support permissions of user_grant to get.");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
...
void PermissionManager::ParamUpdate(const std::string& permissionName, uint32_t flag, bool filtered)
{
    ...
    if (filtered || (PermissionDefinitionCache::GetInstance().IsUserGrantedPermission(permissionName) &&
        ...
        if (res != 0) {
            LOGE(AT_DOMAIN, AT_TAG, "SetParameter failed %{public}d", res);
        }
    }
}
...
{
    LOGI(AT_DOMAIN, AT_TAG, "IsUpdated");
    ...
    AccessTokenID id, const std::string& permission, bool isGranted, uint32_t flag, bool needKill)
{
    ...
    if (infoPtr == nullptr) {
        LOGE(AT_DOMAIN, AT_TAG, "tokenInfo is null, tokenId=%{public}u", id);
        return AccessTokenError::ERR_TOKENID_NOT_EXIST;
    }
    if (infoPtr->IsRemote()) {
        LOGE(AT_DOMAIN, AT_TAG, "Remote token can not update");
    ...
    if ((flag == PERMISSION_ALLOW_THIS_TIME) && isGranted) {
        if (!TempPermissionObserver::GetInstance().IsAllowGrantTempPermission(id, permission)) {
            LOGE(AT_DOMAIN, AT_TAG,
                "Id:%{public}d fail to grant permission:%{public}s", id, permission.c_str());
    ...
    if (hapDlpType != DLP_COMMON) {
        ...
        if (!DlpPermissionSetManager::GetInstance().IsPermDlpModeAvailableToDlpHap(hapDlpType, permDlpMode)) {
            LOGD(AT_DOMAIN, AT_TAG,
                "%{public}s cannot to be granted to %{public}u", permission.c_str(), id);
            return AccessTokenError::ERR_IDENTITY_CHECK_FAILED;
        }
    }
#endif
    ...
    if (statusChanged) {
        ...
        if (needKill && (!isGranted && !isSecCompGrantedBefore)) {
            LOGI(AT_DOMAIN, AT_TAG,
                "(%{public}s) is revoked, kill process(%{public}u).", permission.c_str(), id);
            if ((ret = AppManagerAccessClient::GetInstance().KillProcessesByAccessTokenId(id)) != ERR_OK) {
                LOGE(AT_DOMAIN, AT_TAG, "kill process failed, ret=%{public}d.", ret);
            }
        }
    }
...
{
    if (!PermissionValidator::IsPermissionNameValid(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "Invalid permission: %{public}s!", permissionName.c_str());
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    if (!PermissionDefinitionCache::GetInstance().HasDefinition(permissionName)) {
        LOGE(AT_DOMAIN, AT_TAG, "No definition for permission: %{public}s!", permissionName.c_str());
    ...
    if (!PermissionValidator::IsPermissionFlagValid(flag)) {
        LOGE(AT_DOMAIN, AT_TAG, "Invalid flag: %{public}d!", flag);
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    bool needKill = false;
    // To kill process when perm is revoke
    if (!isGranted && (flag != PERMISSION_ALLOW_THIS_TIME) && (flag != PERMISSION_COMPONENT_SET)) {
        LOGI(AT_DOMAIN, AT_TAG, "Perm(%{public}s) is revoked, kill process(%{public}u).",
...
{
    LOGI(AT_DOMAIN, AT_TAG, "Id: %{public}u, permissionName: %{public}s, flag: %{public}d",
...
{
    LOGI(AT_DOMAIN, AT_TAG, "Id: %{public}u, permissionName: %{public}s, onceTime: %{public}d",
    ...
    const std::vector<AccessTokenID>& tokenIDs, const std::vector<std::string>& permList)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "TokenidStr = %{public}s permStr =%{public}s",
...
int32_t PermissionManager::ScopeFilter(const PermStateChangeScope& scopeSrc, PermStateChangeScope& scopeRes)
{
    ...
    for (const auto& tokenId : scopeSrc.tokenIDs) {
        ...
        LOGE(AT_DOMAIN, AT_TAG, "TokenId %{public}d invalid!", tokenId);
    }
    ...
    for (const auto& permissionName : scopeSrc.permList) {
        ...
        LOGE(AT_DOMAIN, AT_TAG, "Permission %{public}s invalid!", permissionName.c_str());
    }
    if ((scopeRes.tokenIDs.empty()) && (!scopeSrc.tokenIDs.empty())) {
        LOGE(AT_DOMAIN, AT_TAG, "Valid tokenid size is 0!");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
    if ((scopeRes.permList.empty()) && (!scopeSrc.permList.empty())) {
        LOGE(AT_DOMAIN, AT_TAG, "Valid permission size is 0!");
        return AccessTokenError::ERR_PARAM_INVALID;
    }
...
bool PermissionManager::GetApiVersionByTokenId(AccessTokenID tokenID, int32_t& apiVersion)
{
    ...
    if (tokenType != TOKEN_HAP) {
        LOGE(AT_DOMAIN, AT_TAG, "Invalid token type %{public}d", tokenType);
        return false;
    }
    ...
    if (ret != RET_SUCCESS) {
        LOGE(AT_DOMAIN, AT_TAG, "Get hap token info error!");
        return false;
    }
...
{
    if (!PermissionValidator::IsPermissionNameValid(permissionName)) {
        LOGW(AT_DOMAIN, AT_TAG, "Invalid permission %{public}s", permissionName.c_str());
        return false;
    }

    if (!PermissionDefinitionCache::GetInstance().HasDefinition(permissionName)) {
        LOGW(AT_DOMAIN, AT_TAG, "Permission %{public}s has no definition ", permissionName.c_str());
        return false;
    }
    return true;
}
    ...
    const std::vector<std::string>& grantedPermListAfter, AccessTokenID tokenID)
{
    for (uint32_t i = 0; i < grantedPermListBefore.size(); i++) {
        LOGI(AT_DOMAIN, AT_TAG,
            "grantedPermListBefore[i] %{public}s.", grantedPermListBefore[i].c_str());
    ...
    for (uint32_t i = 0; i < grantedPermListAfter.size(); i++) {
        LOGI(AT_DOMAIN, AT_TAG,
            "grantedPermListAfter[i] %{public}s.", grantedPermListAfter[i].c_str());
...
void PermissionManager::AddPermToKernel(AccessTokenID tokenID, const std::shared_ptr<PermissionPolicySet>& policy)
{
    ...
    if (ret != ACCESS_TOKEN_OK) {
        LOGE(AT_DOMAIN, AT_TAG,
            "AddPermToKernel(id=%{public}u), size=%{public}zu, err=%{public}d", tokenID, opCodeList.size(), ret);
    }
}
...
void PermissionManager::AddPermToKernel(AccessTokenID tokenID, const std::vector<std::string>& permList)
{
    ...
    if (ret != ACCESS_TOKEN_OK) {
        LOGE(AT_DOMAIN, AT_TAG,
            "AddPermToKernel(id=%{public}d), size=%{public}zu, err=%{public}d", tokenID, opCodeList.size(), ret);
    }
}
...
void PermissionManager::RemovePermFromKernel(AccessTokenID tokenID)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "RemovePermFromKernel(id=%{public}d), err=%{public}d", tokenID, ret);
}
    ...
    AccessTokenID tokenID, const std::string& permissionName, bool isGranted)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "SetPermToKernel(id=%{public}d, permission=(%{public}s), err=%{public}d",
...
bool IsAclSatisfied(const PermissionDef& permDef, const HapPolicyParams& policy)
{
    if (policy.apl < permDef.availableLevel) {
        if (!permDef.provisionEnable) {
            LOGE(AT_DOMAIN, AT_TAG,
                "%{public}s provisionEnable is false.", permDef.permissionName.c_str());
            return false;
        }
        ...
        if (!isAclExist) {
            LOGE(AT_DOMAIN, AT_TAG, "%{public}s need acl.", permDef.permissionName.c_str());
            return false;
        }
    }
    return true;
...
bool IsPermAvailableRangeSatisfied(const PermissionDef& permDef, const std::string& appDistributionType)
{
    if (permDef.availableType == ATokenAvailableTypeEnum::MDM) {
        if (appDistributionType == "none") {
            LOGI(AT_DOMAIN, AT_TAG, "Debug app use permission: %{public}s.",
                permDef.permissionName.c_str());
            return true;
        }
        if (appDistributionType != APP_DISTRIBUTION_TYPE_ENTERPRISE_MDM) {
            LOGE(AT_DOMAIN, AT_TAG, "%{public}s is mdm permission, the hap is not a mdm application.",
                permDef.permissionName.c_str());
            return false;
        }
    }
    ...
    const std::string &permissionName, bool &userCancelable)
{
    ...
    if (iter == list.end()) {
        LOGI(AT_DOMAIN, AT_TAG, "Permission(%{public}s) is not in the list", permissionName.c_str());
        return false;
    }
    ...
    std::vector<PermissionStateFull>& initializedList)
{
    ...
    if (infoPtr == nullptr) {
        LOGE(AT_DOMAIN, AT_TAG, "Token %{public}u is invalid.", tokenId.tokenIdExStruct.tokenID);
        return false;
    }
...
{
    LOGI(AT_DOMAIN, AT_TAG, "Before, request permListSize: %{public}zu, "
        "preAuthorizationSize %{public}zu, ACLRequestedList size %{public}zu.",
    ...
    for (auto state : policy.permStateList) {
        ...
        if (ret != AccessToken::AccessTokenKitRet::RET_SUCCESS) {
            LOGE(AT_DOMAIN, AT_TAG, "Get definition of %{public}s failed, ret = %{public}d.",
                state.permissionName.c_str(), ret);
            continue;
        }
        if (!IsAclSatisfied(permDef, policy)) {
            LOGE(AT_DOMAIN, AT_TAG, "Acl of %{public}s is invalid.", permDef.permissionName.c_str());
            return false;
        }
        ...
        if (!IsPermAvailableRangeSatisfied(permDef, appDistributionType)) {
            LOGE(AT_DOMAIN, AT_TAG,
                "Available range of %{public}s is invalid.", permDef.permissionName.c_str());
            return false;
        }
    ...
    LOGI(AT_DOMAIN, AT_TAG, "After, request perm list size: %{public}zu.", initializedList.size());
    return true;
}
