...
namespace {
constexpr const char* TASK_NAME_TEMP_PERMISSION = "atm_permission_manager_temp_permission";
constexpr const char* FORM_INVISIBLE_NAME = "#0";
constexpr const char* FORM_VISIBLE_NAME = "#1";
...
void PermissionAppStateObserver::OnAppStateChanged(const AppStateData &appStateData)
{
    ...
    if (!TempPermissionObserver::GetInstance().GetAppStateListByTokenID(tokenID, list)) {
        LOGD(AT_DOMAIN, AT_TAG, "TokenID:%{public}d not use temp permission", tokenID);
        return;
    }
    LOGI(AT_DOMAIN, AT_TAG,
        "OnChange(Id=%{public}d, state=%{public}d)", tokenID, appStateData.state);
    if (appStateData.state == static_cast<int32_t>(ApplicationState::APP_STATE_FOREGROUND)) {
        ...
        if (list[FORMS_FLAG]) {
            LOGW(AT_DOMAIN, AT_TAG, "Id:%{public}d has form, don't delayRevokePermission!", tokenID);
            return;
        }
        if (list[CONTINUOUS_TASK_FLAG]) {
            LOGW(AT_DOMAIN, AT_TAG,
                "Id:%{public}d has continuoustask, don't delayRevokePermission!", tokenID);
            return;
        }
        std::string taskName = TASK_NAME_TEMP_PERMISSION + std::to_string(tokenID);
...
void PermissionAppStateObserver::OnAppStopped(const AppStateData &appStateData)
{
    if (appStateData.state == static_cast<int32_t>(ApplicationState::APP_STATE_TERMINATED)) {
        uint32_t tokenID = appStateData.accessTokenId;
        LOGI(AT_DOMAIN, AT_TAG, "Id:%{public}d died.", tokenID);
    ...
    const std::string &bundleName, std::vector<FormInstance> &formInstances)
{
    for (size_t i = 0; i < formInstances.size(); i++) {
        ...
        std::vector<bool> list;
        if (!TempPermissionObserver::GetInstance().GetAppStateListByTokenID(tokenID, list)) {
            LOGE(AT_DOMAIN, AT_TAG, "Id:%{public}d not use temp permission", tokenID);
            continue;
        }

        LOGI(AT_DOMAIN, AT_TAG, "%{public}s, Id: %{public}d, formVisiblity:%{public}d",
        ...
        if (formInstances[i].formVisiblity_ == FormVisibilityType::VISIBLE) {
            ...
            if (list[FOREGROUND_FLAG]) {
                LOGW(AT_DOMAIN, AT_TAG,
                    "Id:%{public}d in foreground don't delayRevokePermission!", tokenID);
                continue;
            }
            if (list[CONTINUOUS_TASK_FLAG]) {
                LOGW(AT_DOMAIN, AT_TAG,
                    "Id:%{public}d has task, don't delayRevokePermission!", tokenID);
                continue;
            }
            std::string taskName = TASK_NAME_TEMP_PERMISSION + std::to_string(tokenID);
            TempPermissionObserver::GetInstance().DelayRevokePermission(tokenID, taskName);
    ...
    const std::shared_ptr<ContinuousTaskCallbackInfo> &continuousTaskCallbackInfo)
{
    ...
    if (static_cast<BackgroundMode>(typeId) != BackgroundMode::LOCATION) {
        LOGD(AT_DOMAIN, AT_TAG, "TypeId:%{public}d can not use temp permission", typeId);
        return;
    }
    std::vector<bool> list;
    if (!TempPermissionObserver::GetInstance().GetAppStateListByTokenID(tokenID, list)) {
        LOGD(AT_DOMAIN, AT_TAG, "Id:%{public}d not use temp permission", tokenID);
        return;
    }
    LOGI(AT_DOMAIN, AT_TAG, "%{public}d", tokenID);
    ...
    const std::shared_ptr<ContinuousTaskCallbackInfo> &continuousTaskCallbackInfo)
{
    ...
    if (TempPermissionObserver::GetInstance().FindContinuousTask(tokenID)) {
        LOGW(AT_DOMAIN, AT_TAG, "Has continuous task don't delayRevokePermission!");
        return;
    }
    ...
    if (list[FOREGROUND_FLAG] || list[FORMS_FLAG]) {
        LOGW(AT_DOMAIN, AT_TAG, "Has form or inForeground don't delayRevokePermission!");
        return;
    }
    std::string taskName = TASK_NAME_TEMP_PERMISSION + std::to_string(tokenID);
    TempPermissionObserver::GetInstance().DelayRevokePermission(tokenID, taskName);
...
{
    LOGI(AT_DOMAIN, AT_TAG, "TempPermissionObserver AppManagerDeath called");
...
void TempPermissionObserver::RegisterCallback()
{
#ifdef BGTASKMGR_CONTINUOUS_TASK_ENABLE
    {
        ...
        if (backgroundTaskCallback_ == nullptr) {
            ...
            if (backgroundTaskCallback_ == nullptr) {
                LOGE(AT_DOMAIN, AT_TAG, "Register backgroundTaskCallback failed.");
                return;
            }
            int ret = BackgourndTaskManagerAccessClient::GetInstance().SubscribeBackgroundTask(backgroundTaskCallback_);
            LOGI(AT_DOMAIN, AT_TAG, "Register backgroundTaskCallback %{public}d.", ret);
        }
    }
#endif
    {
...
{
    {
        ...
        if (appStateCallback_ == nullptr) {
            ...
            if (appStateCallback_ == nullptr) {
                LOGE(AT_DOMAIN, AT_TAG, "Register appStateCallback failed.");
                return;
            }
            int ret = AppManagerAccessClient::GetInstance().RegisterApplicationStateObserver(appStateCallback_);
            if (ret != ERR_OK) {
                LOGI(AT_DOMAIN, AT_TAG, "Register appStateCallback %{public}d.", ret);
            }
        }
    }
    // app manager death callback register
    {
        ...
        if (appManagerDeathCallback_ == nullptr) {
            ...
            if (appManagerDeathCallback_ == nullptr) {
                LOGE(AT_DOMAIN, AT_TAG, "Register appManagerDeathCallback failed.");
                return;
            }
...
{
    {
        ...
        if (appStateCallback_ != nullptr) {
            ...
            if (ret != ERR_OK) {
                LOGI(AT_DOMAIN, AT_TAG, "Unregister appStateCallback %{public}d.", ret);
            }
...
void TempPermissionObserver::ModifyAppState(AccessTokenID tokenID, int32_t index, bool flag)
{
    ...
    auto iter = tempPermTokenMap_.find(tokenID);
    if (iter == tempPermTokenMap_.end()) {
        LOGE(AT_DOMAIN, AT_TAG, "TokenID:%{public}d not exist in map", tokenID);
        return;
    }
...
bool TempPermissionObserver::GetTokenIDByBundle(const std::string &bundleName, AccessTokenID& tokenID)
{
    ...
    if (iter == formTokenMap_.end()) {
        LOGD(AT_DOMAIN, AT_TAG, "BundleName:%{public}s not exist in map", bundleName.c_str());
        return false;
    }
    tokenID = iter->second;
    return true;
...
void TempPermissionObserver::AddContinuousTask(AccessTokenID tokenID)
{
    ...
    if (iter == continuousTaskMap_.end()) {
        LOGI(AT_DOMAIN, AT_TAG, "TokenID:%{public}d not exist in map", tokenID);
...
void TempPermissionObserver::DelContinuousTask(AccessTokenID tokenID)
{
    ...
    continuousTaskMap_[tokenID]--;
    LOGI(AT_DOMAIN, AT_TAG, "TokenID:%{public}d has %{public}d tasks in map",
...
bool TempPermissionObserver::IsAllowGrantTempPermission(AccessTokenID tokenID, const std::string& permissionName)
{
    ...
    if (AccessTokenInfoManager::GetInstance().GetHapTokenInfo(tokenID, tokenInfo) != RET_SUCCESS) {
        LOGE(AT_DOMAIN, AT_TAG, "Invalid Id(%{public}d)", tokenID);
        return false;
    }
    auto iterator = std::find(g_tempPermission.begin(), g_tempPermission.end(), permissionName);
    if (iterator == g_tempPermission.end()) {
        LOGW(AT_DOMAIN, AT_TAG,
            "Permission is not available to temp grant: %{public}s!", permissionName.c_str());
        return false;
    }
    ...
    const std::string& permissionName, const std::string& bundleName)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "Id:%{public}d, isForeground:%{public}d, isFormVisible:%{public}d,"
    ...
    const std::string& bundleName, const std::string& permissionName, const std::vector<bool>& list)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "Id:%{public}d, permission:%{public}s", tokenID, permissionName.c_str());
    ...
    std::vector<PermissionStateFull>& permissionStateFullList)
{
    ...
    if (infoPtr->IsRemote()) {
        LOGE(AT_DOMAIN, AT_TAG, "It is a remote hap token %{public}u!", tokenID);
        return false;
    }
    if (infoPtr->GetPermissionStateList(permissionStateFullList) != RET_SUCCESS) {
        LOGE(AT_DOMAIN, AT_TAG, "GetPermissionStateList failed, token %{public}u!", tokenID);
        return false;
    }
    return true;
}
...
void TempPermissionObserver::RevokeAllTempPermission(AccessTokenID tokenID)
{
    ...
    if (!GetAppStateListByTokenID(tokenID, list)) {
        LOGE(AT_DOMAIN, AT_TAG, "TokenID:%{public}d not exist in permList", tokenID);
        return;
    }
    std::unique_lock<std::mutex> lck(tempPermissionMutex_);
    ...
    if (!GetPermissionStateFull(tokenID, tmpList)) {
        LOGE(AT_DOMAIN, AT_TAG, "TokenID:%{public}d get permission state full fail!", tokenID);
        return;
    }
    for (const auto& permissionState : tmpList) {
        if (permissionState.grantFlags[0] & PERMISSION_ALLOW_THIS_TIME) {
            if (PermissionManager::GetInstance().RevokePermission(
                ...
                LOGE(AT_DOMAIN, AT_TAG, "TokenID:%{public}d revoke permission:%{public}s failed!",
...
void TempPermissionObserver::OnAppMgrRemoteDiedHandle()
{
    ...
    for (auto iter = tempPermTokenMap_.begin(); iter != tempPermTokenMap_.end(); ++iter) {
        ...
        for (const auto& permissionState : tmpList) {
            ...
            if (ret != RET_SUCCESS) {
                LOGE(AT_DOMAIN, AT_TAG, "revoke permission failed, TokenId=%{public}d, permission \
    ...
    LOGI(AT_DOMAIN, AT_TAG, "TempPermTokenMap_ clear!");
    appStateCallback_= nullptr;
}
...
bool TempPermissionObserver::DelayRevokePermission(AccessToken::AccessTokenID tokenID, const std::string& taskName)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "Add permission task name:%{public}s", taskName.c_str());

    std::function<void()> delayed = ([tokenID]() {
        TempPermissionObserver::GetInstance().RevokeAllTempPermission(tokenID);
        LOGI(AT_DOMAIN, AT_TAG, "Token: %{public}d, delay revoke permission end", tokenID);
    });
    eventHandler->ProxyPostTask(delayed, taskName, cancleTimes_);
    return true;
#else
    LOGW(AT_DOMAIN, AT_TAG, "Eventhandler is not existed");
    return false;
#endif
}
...
bool TempPermissionObserver::CancleTaskOfPermissionRevoking(const std::string& taskName)
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "Revoke permission task name:%{public}s", taskName.c_str());
...
void TempPermissionObserver::GetConfigValue()
{
    ...
    LOGI(AT_DOMAIN, AT_TAG, "CancleTimes_ is %{public}d.", cancleTimes_);
}
} // namespace AccessToken
} // namespace Security
} // namespace OHOS
