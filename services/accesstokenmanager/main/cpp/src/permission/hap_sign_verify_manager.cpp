...
bool IsEnterpriseDistributionType(Security::Verify::AppDistType type)
{
    return type == Security::Verify::ENTERPRISE || type == Security::Verify::ENTERPRISE_NORMAL ||
        type == Security::Verify::ENTERPRISE_MDM;
}
}
...
bool RdDeviceChecker::CheckDeviceMode(char *buf, ssize_t bufLen)
{
    ...
    if (onStr != nullptr && offStr != nullptr) {
        LOGE(ATM_DOMAIN, ATM_TAG, "cmdline attacked, both rd and user found, cmdline = %{private}s", buf);
...
bool RdDeviceChecker::CheckEfuseStatus(char *buf, ssize_t bufLen)
{
    ...
    if (onStr != nullptr && offStr != nullptr) {
        LOGE(ATM_DOMAIN, ATM_TAG, "cmdline attacked, both efused and not efused found, cmdline = %{private}s", buf);
    } else if (onStr == nullptr && offStr == nullptr) {
    ...
    const Security::Verify::ProvisionInfo& provisionInfo, TrustedBundleInfoInner& info) const
{
    ...
    if (moduleInfo.apiTargetVersion < 0) {
        LOGE(ATM_DOMAIN, ATM_TAG, "Invalid apiTargetVersion=%{public}d.", moduleInfo.apiTargetVersion);
        return AccessTokenError::ERR_PARAM_INVALID;
    }
...
std::string TrustedBundleInfoInner::GetAppFeature() const
{
    return provisionInfo.bundleInfo.appFeature;
}
    ...
    TrustedBundleInfoInner& info, bool& isChanged) const
{
    ...
    if (realpath(path.c_str(), resolvedPath) == nullptr) {
        if (errno != ENOENT) {
            LOGC(ATM_DOMAIN, ATM_TAG, "Failed to resolve path %{private}s: %{public}s", path.c_str(), strerror(errno));
            return ERR_VERIFY_CANT_OPEN;
        } else {
            LOGW(ATM_DOMAIN, ATM_TAG, "Failed to resolve path %{private}s: %{public}s", path.c_str(), strerror(errno));
            return ERR_VERIFY_FILE_NOT_EXIST;
        }
    }

    if (!IsHapPathAllowed(resolvedPath)) {
        LOGW(ATM_DOMAIN, ATM_TAG, "Hap path is not in allowed directories, path=%{private}s.", resolvedPath);
    }
...
int32_t HapSignVerifyManager::CheckMultipleHaps(const std::vector<TrustedBundleInfoInner>& infos) const
{
    ...
    bool isInvalid = std::any_of(infos.begin(), infos.end(), [&baseline, this](const auto& info) {
        ...
        if (baseline.GetAppFeature() != info.GetAppFeature()) {
            LOGC(ATM_DOMAIN, ATM_TAG, "Check multiple haps failed, hap files have different appFeature.");
            return true;
        }
        if (IsEnterpriseDistributionType(baseline.provisionInfo.distributionType) !=
            IsEnterpriseDistributionType(info.provisionInfo.distributionType)) {
            LOGC(ATM_DOMAIN, ATM_TAG,
                "Check multiple haps failed, hap files have different appDistributionType class.");
            return true;
        }
