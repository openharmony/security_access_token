...
int32_t AddSpmDataTask::Add(uint32_t& errIndex)
{
    ...
    uint8_t idxErr = 0;
    int32_t ret = KernelDetail::AddSpmEntriesToKernel(entries, tokenIds, idxErr);
    if (ret == ERR_IOCTL_UNSUPPORT) {
        LOGW(ATM_DOMAIN, ATM_TAG, "Add spm entries is not supported in current system.");
    ...
    ret = AddPermDataToKernel(items_, permDataSuccessCount_, errIndex);
    if (ret != RET_SUCCESS) {
        (void)Rollback();
        return ret;
    }
