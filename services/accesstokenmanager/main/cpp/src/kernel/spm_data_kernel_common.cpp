...
int32_t ParseExtendedPermissionBuffer(const SpmBlob& extendPermBlob, std::vector<PermissionWithValue>& permList)
{
    ...
    while (offset <= extendPermBlob.bufSize - sizeof(uint32_t) - sizeof(uint32_t)) {
        uint32_t permCode = 0;
        (void)memcpy_s(&permCode, sizeof(permCode), extendPermBlob.buf + offset, sizeof(uint32_t));
        offset += sizeof(uint32_t);
        uint32_t valueSize = 0;
        (void)memcpy_s(&valueSize, sizeof(valueSize), extendPermBlob.buf + offset, sizeof(uint32_t));
        offset += sizeof(uint32_t);
        ...
        if (extendPermBlob.buf[offset + valueSize - 1] != '\0') {
            LOGE(ATM_DOMAIN, ATM_TAG, "Value end is not null, permCode=%{public}u, force terminated.", permCode);
            extendPermBlob.buf[offset + valueSize - 1] = '\0';
        }
    ...
    SpmDataPtr& spmData)
{
    ...
    if (extendPermsBuffer.size() > DEFAULT_EXTENDED_PERMS) {
        // buffer beyond the kernel read limit can never be loaded back by SpmGetEntry
        LOGE(ATM_DOMAIN, ATM_TAG,
            "Extend perms buffer %{public}zu exceeds kernel read limit %{public}u, tokenId=%{public}u.",
            extendPermsBuffer.size(), DEFAULT_EXTENDED_PERMS, hapInfo.tokenID);
        return ERR_OVERSIZE;
    }
...
int32_t AddSpmEntriesToKernel(const std::vector<SpmData*>& entries, uint8_t& idxErr)
{
    ...
    // entries before idxErr have been added successfully, retry only the failed part
    uint8_t failedIdx = idxErr;
    if (failedIdx >= entries.size()) {
        LOGE(ATM_DOMAIN, ATM_TAG, "SpmAddEntries failed at invalid index=%{public}u, ret=%{public}d.", failedIdx, ret);
        return ERR_KERNEL_COMMON_FAILED;
    }
    ret = SpmAddEntries(const_cast<SpmData**>(entries.data()) + failedIdx,
        static_cast<uint8_t>(entries.size() - failedIdx), &idxErr);
    if (ret == RET_SUCCESS) {
        return RET_SUCCESS;
    }
    LOGE(ATM_DOMAIN, ATM_TAG, "SpmAddEntries retry failed at index=%{public}u, ret=%{public}d.",
        failedIdx + idxErr, ret);
    idxErr = failedIdx + idxErr;
    return ERR_KERNEL_COMMON_FAILED;
}
...
int32_t SetSpmEntriesToKernel(const std::vector<SpmData*>& entries, uint8_t& idxErr)
{
    ...
    // entries before idxErr have been set successfully, retry only the failed part
    uint8_t failedIdx = idxErr;
    if (failedIdx >= entries.size()) {
        LOGE(ATM_DOMAIN, ATM_TAG, "SpmSetEntries failed at invalid index=%{public}u, ret=%{public}d.", failedIdx, ret);
        return ERR_KERNEL_COMMON_FAILED;
    }
    ret = SpmSetEntries(const_cast<SpmData**>(entries.data()) + failedIdx,
        static_cast<uint8_t>(entries.size() - failedIdx), &idxErr);
    if (ret == RET_SUCCESS) {
        return RET_SUCCESS;
    }
    LOGE(ATM_DOMAIN, ATM_TAG, "SpmSetEntries retry failed at index=%{public}u, ret=%{public}d.",
        failedIdx + idxErr, ret);
    idxErr = failedIdx + idxErr;
    return ERR_KERNEL_COMMON_FAILED;
}
...
int32_t LoadSpmDataFromKernel(AccessTokenID tokenId, SpmDataPtr& spmData)
{
    ...
    if (ret == ENODATA) {
        LOGI(ATM_DOMAIN, ATM_TAG, "SpmGetEntry none for token=%{public}u.", tokenId);
...
int32_t RemoveSpmEntryFromKernel(AccessTokenID tokenId)
{
    ...
    // ENOTSUP: removal is best-effort for rollback; ENODATA: entry absent, removal is idempotent
    if (ret == RET_SUCCESS || ret == ENOTSUP || ret == ENODATA) {
        return RET_SUCCESS;
    }
    // retry once for the failed entry
