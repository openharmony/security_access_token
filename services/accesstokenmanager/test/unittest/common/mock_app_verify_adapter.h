...
public:
    ...
    int32_t ParseProvision(const std::string& appProvision, Security::Verify::ProvisionInfo& info) const override
    {
        ...
        if (parseProvisionRet_ != RET_SUCCESS) {
            return parseProvisionRet_;
        }
        info.versionCode = 1;
    ...
    int32_t ParseProfile(const std::string& appProvision, Security::Verify::ProvisionInfo& info) const override
    {
        ...
        if (parseProfileRet_ != RET_SUCCESS) {
            return parseProfileRet_;
        }
        info.versionCode = 1;
        info.type = provisionType_;
        info.distributionType = distributionType_;
    ...
    int32_t parseProvisionRet_ = RET_SUCCESS;
    int32_t parseProfileRet_ = RET_SUCCESS;
