...
int32_t AddPermissionToKernel(uint32_t tokenID, const std::vector<uint32_t>& opCodeList)
{
    ...
    state.addPermOpCodeBatches.emplace_back(opCodeList);
...
int SpmAddEntries(SpmData **entries, uint8_t cnt, uint8_t *idxErr)
{
    ...
    state.addTokenBatches.emplace_back(tokenIds);
    state.addCallCount++;
    if (idxErr != nullptr) {
        // clamp to a valid relative index like a sane kernel would report
        *idxErr = (cnt > 0 && state.addIdxErr >= cnt) ? static_cast<uint8_t>(cnt - 1) : state.addIdxErr;
    }
...
int SpmSetEntries(SpmData **entries, uint8_t cnt, uint8_t *idxErr)
{
    ...
    if (idxErr != nullptr) {
        // clamp to a valid relative index like a sane kernel would report
        *idxErr = (cnt > 0 && state.setIdxErr >= cnt) ? static_cast<uint8_t>(cnt - 1) : state.setIdxErr;
    }
...
int SpmGetEntry(uint32_t tokenid, SpmData *entry)
{
    ...
    // Set bufSize to 0 to skip extended permissions parsing (avoiding ERR_PARAM_INVALID)
...
int SpmRemoveEntry(uint32_t tokenid)
{
    ...
    if (static_cast<size_t>(state.removeCallCount) <= state.removeRetSequence.size()) {
        return state.removeRetSequence[state.removeCallCount - 1];
    }
...
{
    auto& state = GetFakeSpmKernelState();
    if (state.getVersionRet != 0) {
        return state.getVersionRet;
    }
    if (version != nullptr) {
        *version = 1;
    }
