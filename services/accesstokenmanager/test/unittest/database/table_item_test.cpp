...
HWTEST_F(TableItemTest, BundleInfoItemsBuildAddValue001, TestSize.Level0)
{
    ...
    EXPECT_EQ(expectedData, value.GetBlob(TokenFiledConst::FIELD_PERSIST_DATA));
}
...
HWTEST_F(TableItemTest, BundleInfoItemsLoadFromDB001, TestSize.Level0)
{
    ...
    std::vector<uint8_t> expectedData = {0x01, 0x02};
    EXPECT_EQ(expectedData, items[0].moduleInfoItems[0].persistData);
}
...
/**
 * @tc.name: HapTokenInfoItemGenerateAddList001
 * @tc.desc: Verify HapTokenInfoItem::GenerateAddList generates correct AddInfo.
 * @tc.type: FUNC
 */
HWTEST_F(TableItemTest, HapTokenInfoItemGenerateAddList001, TestSize.Level0)
{
    std::vector<GenericValues> values;
    GenericValues value;
    value.Put(TokenFiledConst::FIELD_TOKEN_ID, static_cast<int32_t>(0x12345678));
    values.emplace_back(value);

    std::vector<AddInfo> addInfoVec;
    HapTokenInfoItem::GenerateAddList(values, addInfoVec);

    ASSERT_EQ(1u, addInfoVec.size());
    EXPECT_EQ(HapTokenInfoItem::type_, addInfoVec[0].addType);
    ASSERT_EQ(1u, addInfoVec[0].addValues.size());
}
