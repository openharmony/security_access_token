...
/**
 * @tc.name: ExecuteSpmKernelTasks002
 * @tc.desc: Test ExecuteSpmKernelTasks with RESERVED tokenID, should add to addParams
 * @tc.type: FUNC
 * @tc.require: Issue
 */
HWTEST_F(InstallSessionSpmTaskTest, ExecuteSpmKernelTasks002, TestSize.Level0)
{
    // Create a tokenID and set it to RESERVED status
    AccessTokenID tokenId1 = 0x20000002;
    int32_t ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId1, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);
    ret = AccessTokenIDManager::GetInstance().ChangeTokenIdStatus(tokenId1, TokenIdStatus::RESERVED);
    ASSERT_EQ(ERR_OK, ret);
    ...
    HapTokenInfo hapInfo1 = BuildHapInfo(tokenId1, "test.bundle");
    context.hapInfos.emplace_back(hapInfo1);
    context.permBriefDataLists.emplace_back(BuildBriefList(2));
    context.extendPermLists.emplace_back();
    context.oldPermList.emplace_back(std::make_shared<std::vector<BriefPermData>>());
    ...
    // Verify: RESERVED tokenID should be added to addParams, not updateParams
    EXPECT_EQ(RET_SUCCESS, ret);
    EXPECT_EQ(1, spmContext.addParams.size());
    EXPECT_EQ(0, spmContext.updateParams.size());

    AccessTokenIDManager::GetInstance().ReleaseTokenId(tokenId1);
}

/**
 * @tc.name: ExecuteSpmKernelTasks003
 * @tc.desc: Test ExecuteSpmKernelTasks with UNTRUSTED tokenID, should add to addParams
 * @tc.type: FUNC
 * @tc.require: Issue
 */
HWTEST_F(InstallSessionSpmTaskTest, ExecuteSpmKernelTasks003, TestSize.Level0)
{
    // Create a tokenID and set it to UNTRUSTED status
    AccessTokenID tokenId1 = 0x20000003;
    int32_t ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId1, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);
    ret = AccessTokenIDManager::GetInstance().ChangeTokenIdStatus(tokenId1, TokenIdStatus::UNTRUSTED);
    ASSERT_EQ(ERR_OK, ret);
    ...
    HapTokenInfo hapInfo1 = BuildHapInfo(tokenId1, "test.bundle");
    context.hapInfos.emplace_back(hapInfo1);
    context.permBriefDataLists.emplace_back(BuildBriefList(3));
    context.extendPermLists.emplace_back();
    context.oldPermList.emplace_back(std::make_shared<std::vector<BriefPermData>>());
    ...
    // Verify: UNTRUSTED tokenID should be added to addParams, not updateParams
    EXPECT_EQ(RET_SUCCESS, ret);
    EXPECT_EQ(1, spmContext.addParams.size());
    EXPECT_EQ(0, spmContext.updateParams.size());
...
/**
 * @tc.name: ExecuteSpmKernelTasks004
 * @tc.desc: Test ExecuteSpmKernelTasks with mixed ACTIVE and non-ACTIVE tokenIDs
 * @tc.type: FUNC
 * @tc.require: Issue
 */
HWTEST_F(InstallSessionSpmTaskTest, ExecuteSpmKernelTasks004, TestSize.Level0)
{
    // Create three tokenIDs with different statuses
    AccessTokenID tokenId1 = 0x20000004;
    AccessTokenID tokenId2 = 0x20000005;
    AccessTokenID tokenId3 = 0x20000006;

    int32_t ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId1, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);
    ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId2, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);
    ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId3, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);

    // Set tokenId2 to RESERVED, tokenId3 stays ACTIVE
    ret = AccessTokenIDManager::GetInstance().ChangeTokenIdStatus(tokenId2, TokenIdStatus::RESERVED);
    ASSERT_EQ(ERR_OK, ret);
    ...
    context.hapInfos.emplace_back(BuildHapInfo(tokenId2, "test.bundle"));
    context.hapInfos.emplace_back(BuildHapInfo(tokenId3, "test.bundle"));

    context.permBriefDataLists.emplace_back(BuildBriefList(4));
    context.permBriefDataLists.emplace_back(BuildBriefList(5));
    context.permBriefDataLists.emplace_back(BuildBriefList(6));
    ...
    // Verify: ACTIVE (tokenId1, tokenId3) go to updateParams
    //         RESERVED (tokenId2) goes to addParams
    EXPECT_EQ(RET_SUCCESS, ret);
    EXPECT_EQ(1, spmContext.addParams.size());
    EXPECT_EQ(2, spmContext.updateParams.size());

    AccessTokenIDManager::GetInstance().ReleaseTokenId(tokenId1);
    AccessTokenIDManager::GetInstance().ReleaseTokenId(tokenId2);
    AccessTokenIDManager::GetInstance().ReleaseTokenId(tokenId3);
}
...
/**
 * @tc.name: ExecuteSpmKernelTasks006
 * @tc.desc: Test ExecuteSpmKernelTasks when GetTokenIdStatus fails
 * @tc.type: FUNC
 * @tc.require: Issue
 */
HWTEST_F(InstallSessionSpmTaskTest, ExecuteSpmKernelTasks006, TestSize.Level0)
{
    // Use a tokenID that hasn't been registered (GetTokenIdStatus will fail)
    AccessTokenID invalidTokenId = 0x20000009;

    // Build test data
    InstallCache cache = BuildTestCache();
    FinishContext context = BuildFinishContext();

    context.hapInfos.emplace_back(BuildHapInfo(invalidTokenId, "test.bundle"));
    context.permBriefDataLists.emplace_back(BuildBriefList(9));
    context.extendPermLists.emplace_back();
    context.oldPermList.emplace_back(std::make_shared<std::vector<BriefPermData>>());

    SpmTaskContext spmContext;
    int32_t ret = g_installSessionManager->ExecuteSpmKernelTasks(0, cache, context, spmContext);

    // Verify: When GetTokenIdStatus fails, tokenID should be added to addParams
    EXPECT_EQ(RET_SUCCESS, ret);
    EXPECT_EQ(1, spmContext.addParams.size());
    EXPECT_EQ(0, spmContext.updateParams.size());
}
...
/**
 * @tc.name: ExecuteSpmKernelTasks007
 * @tc.desc: Test ExecuteSpmKernelTasks with multiple UNTRUSTED tokenIDs
 * @tc.type: FUNC
 * @tc.require: Issue
 */
HWTEST_F(InstallSessionSpmTaskTest, ExecuteSpmKernelTasks007, TestSize.Level0)
{
    // Create multiple tokenIDs with UNTRUSTED status
    AccessTokenID tokenId1 = 0x2000000A;
    AccessTokenID tokenId2 = 0x2000000B;
    ...
    ret = AccessTokenIDManager::GetInstance().RegisterTokenId(tokenId2, TOKEN_HAP);
    ASSERT_EQ(ERR_OK, ret);
    ret = AccessTokenIDManager::GetInstance().ChangeTokenIdStatus(tokenId2, TokenIdStatus::UNTRUSTED);
    ASSERT_EQ(ERR_OK, ret);
    ...
    context.permBriefDataLists.emplace_back(BuildBriefList(10));
    context.permBriefDataLists.emplace_back(BuildBriefList(11));
    ...
    // Verify: All UNTRUSTED tokenIDs should go to addParams
    EXPECT_EQ(RET_SUCCESS, ret);
    EXPECT_EQ(2, spmContext.addParams.size());
    EXPECT_EQ(0, spmContext.updateParams.size());
