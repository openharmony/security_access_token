    ...
    const std::string& commandName, const std::string& commandJsonString, const std::string& rawDeviceId)
{
    ...
    if (commandName == SYNC_HAP_COMMAND_NAME) {
        return std::make_shared<SyncRemoteHapTokenCommand>(commandJsonString, rawDeviceId);
    }
    if (commandName == DELETE_TOKEN_COMMAND_NAME) {
        return std::make_shared<DeleteRemoteTokenCommand>(commandJsonString, rawDeviceId);
    }
    if (commandName == UPDATE_HAP_COMMAND_NAME) {
        return std::make_shared<UpdateRemoteHapTokenCommand>(commandJsonString, rawDeviceId);
    }
    return nullptr;
}
}  // namespace AccessToken
