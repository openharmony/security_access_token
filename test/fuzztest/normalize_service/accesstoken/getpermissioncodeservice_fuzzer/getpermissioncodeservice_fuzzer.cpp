...
#include "getpermissioncodeservice_fuzzer.h"
...
namespace OHOS {
    bool GetPermissionCodeServiceFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(
            IAccessTokenManagerIpcCode::COMMAND_GET_PERMISSION_CODE);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetPermissionCodeServiceFuzzTest(data, size);
    return 0;
}