...
#include "addremotepermissionusedrecordasyncstub_fuzzer.h"
...
namespace OHOS {
    bool AddRemotePermissionUsedRecordAsyncStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        infoParcel.info.deviceId = provider.ConsumeRandomLengthString();
        infoParcel.info.deviceName = provider.ConsumeRandomLengthString();
        ...
        uint32_t code = static_cast<uint32_t>(IPrivacyManagerIpcCode::COMMAND_ADD_REMOTE_PERMISSION_USED_RECORD_ASYNC);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::AddRemotePermissionUsedRecordAsyncStubFuzzTest(data, size);
    return 0;
}
