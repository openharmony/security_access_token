...
#include "getdisablepolicystub_fuzzer.h"
...
namespace OHOS {
    bool GetDisablePolicyStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(
            IPrivacyManagerIpcCode::COMMAND_GET_DISABLE_POLICY);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetDisablePolicyStubFuzzTest(data, size);
    return 0;
}
