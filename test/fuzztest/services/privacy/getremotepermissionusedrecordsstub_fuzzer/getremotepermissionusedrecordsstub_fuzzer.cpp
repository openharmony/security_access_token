...
#include "getremotepermissionusedrecordsstub_fuzzer.h"
...
namespace OHOS {
    bool GetRemotePermissionUsedRecordsStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(IPrivacyManagerIpcCode::COMMAND_GET_REMOTE_PERMISSION_USED_RECORDS);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetRemotePermissionUsedRecordsStubFuzzTest(data, size);
    return 0;
}
