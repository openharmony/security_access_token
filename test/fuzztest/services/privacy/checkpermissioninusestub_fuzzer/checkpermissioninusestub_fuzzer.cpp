...
#include "checkpermissioninusestub_fuzzer.h"
...
namespace OHOS {
    bool CheckPermissionInUseStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(
            IPrivacyManagerIpcCode::COMMAND_CHECK_PERMISSION_IN_USE);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::CheckPermissionInUseStubFuzzTest(data, size);
    return 0;
}
