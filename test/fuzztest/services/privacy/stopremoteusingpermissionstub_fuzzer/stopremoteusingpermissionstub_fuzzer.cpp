...
#include "stopremoteusingpermissionstub_fuzzer.h"
...
namespace OHOS {
    bool StopRemoteUsingPermissionStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(IPrivacyManagerIpcCode::COMMAND_STOP_REMOTE_USING_PERMISSION);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::StopRemoteUsingPermissionStubFuzzTest(data, size);
    return 0;
}
