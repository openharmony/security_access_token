...
#include "addremotepermissionusedrecordstub_fuzzer.h"
...
namespace OHOS {
    bool AddRemotePermissionUsedRecordStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        uint32_t code = static_cast<uint32_t>(IPrivacyManagerIpcCode::COMMAND_ADD_REMOTE_PERMISSION_USED_RECORD);
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::AddRemotePermissionUsedRecordStubFuzzTest(data, size);
    return 0;
}
