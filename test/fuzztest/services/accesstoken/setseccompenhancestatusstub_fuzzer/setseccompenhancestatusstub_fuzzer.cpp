...
#include "setseccompenhancestatusstub_fuzzer.h"
...
namespace OHOS {
bool SetSecCompEnhanceStatusStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    if (provider.ConsumeBool()) {
        (void)sendData.WriteBool(provider.ConsumeBool());
    }

    MessageParcel reply;
    MessageOption option;
    uint32_t code = static_cast<uint32_t>(IAccessTokenManagerIpcCode::COMMAND_SET_SEC_COMP_ENHANCE_STATUS);
...
{
    OHOS::SetSecCompEnhanceStatusStubFuzzTest(data, size);
    return 0;
}
