...
#include "gethaptokenidstub_fuzzer.h"
...
namespace OHOS {
    bool GetHapTokenIDStubFuzzTest(const uint8_t* data, size_t size)
    {
        ...
        if (!sendData.WriteUint32(tokenId) || !sendData.WriteInt32(userID) ||
            !sendData.WriteString(bundleName) || !sendData.WriteInt32(instIndex)) {
            return false;
        }
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    OHOS::GetHapTokenIDStubFuzzTest(data, size);
    return 0;
}
