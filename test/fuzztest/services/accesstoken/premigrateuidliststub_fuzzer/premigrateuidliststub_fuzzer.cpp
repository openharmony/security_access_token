...
#include "premigrateuidliststub_fuzzer.h"
...
namespace {
constexpr int32_t MAX_UID_LIST_SIZE = 64;
...
namespace OHOS {
bool PreMigrateUIDListStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    int32_t uidListSize = provider.ConsumeIntegralInRange<int32_t>(-1, MAX_UID_LIST_SIZE);
    if (!datas.WriteInt32(uidListSize)) {
        return false;
    }
    for (int32_t i = 0; i < uidListSize; ++i) {
        if (!datas.WriteInt32(provider.ConsumeIntegral<int32_t>())) {
            return false;
        }
    }

    uint32_t code = static_cast<uint32_t>(IAccessTokenManagerIpcCode::COMMAND_PRE_MIGRATE_U_I_D_LIST);
    MessageParcel reply;
...
{
    OHOS::PreMigrateUIDListStubFuzzTest(data, size);
    return 0;
}
