...
#include "getskillpermissionsstub_fuzzer.h"
...
const std::string DEFAULT_CALLER_BUNDLE = "getskillpermissionsstub.fuzzer";
...
bool GetSkillPermissionsStubFuzzTest(const uint8_t* data, size_t size)
{
    ...
    DelayedSingleton<AccessTokenManagerService>::GetInstance()->OnRemoteRequest(
        static_cast<uint32_t>(IAccessTokenManagerIpcCode::COMMAND_GET_SKILL_PERMISSIONS),
        datas, reply, option);
    return true;
}
}
...
{
    OHOS::GetSkillPermissionsStubFuzzTest(data, size);
    return 0;
}
