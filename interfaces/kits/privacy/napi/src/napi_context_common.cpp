...
PermActiveChangeContext::~PermActiveChangeContext()
{
    if (callbackRef != nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "------~PermActiveChangeContext callback");
        napi_delete_reference(env, callbackRef);
        callbackRef = nullptr;
    }
}
...
{
    ACCESSTOKEN_LOG_ERROR(LABEL, "------ActiveStatusChangeCallback begin");
...
void UvQueueWorkActiveStatusChange(uv_work_t* work, int status)
{
    ...
    ACCESSTOKEN_LOG_ERROR(LABEL, "------UvQueueWorkActiveStatusChange end");
}

bool ConvertActiveChangeResponse(napi_env env, napi_value value, const ActiveChangeResponse& result)
{
    ACCESSTOKEN_LOG_ERROR(LABEL,
        "------ConvertActiveChangeResponse : result 1: tokenID = %{public}d, permissionName = %{public}s, deviceId = %{public}s, type = %{public}d",
        result.tokenID, result.permissionName.c_str(), result.deviceId.c_str(), result.type);
    napi_value element;
    ACCESSTOKEN_LOG_ERROR(LABEL, "------napi_create_uint32(env, result.tokenID, &element)");
    NAPI_CALL_BASE(env, napi_create_uint32(env, result.tokenID, &element), false);
    ACCESSTOKEN_LOG_ERROR(LABEL,
        "------ConvertActiveChangeResponse : result 2: tokenID = %{public}d, permissionName = %{public}s, deviceId = %{public}s, type = %{public}d",
        result.tokenID, result.permissionName.c_str(), result.deviceId.c_str(), result.type);
    ACCESSTOKEN_LOG_ERROR(LABEL, "------napi_set_named_property");
    NAPI_CALL_BASE(env, napi_set_named_property(env, value, "tokenId", element), false);
    element = nullptr;
    ACCESSTOKEN_LOG_ERROR(LABEL, "------napi_create_string_utf8(env, result.permissionName.c_str()");
    ...
    element = nullptr;
    ACCESSTOKEN_LOG_ERROR(LABEL, "------napi_create_string_utf8(env, result.deviceId.c_str()");
    ...
    element = nullptr;
    ACCESSTOKEN_LOG_ERROR(LABEL, "------napi_create_int32(env, result.type, &element)");
