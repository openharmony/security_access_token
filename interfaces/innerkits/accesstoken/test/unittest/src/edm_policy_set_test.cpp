...
public:
    ...
    virtual void PermStateChangeCallback(PermStateChangeInfo& result)
    {
        ...
        EXPECT_EQ(status, AccessTokenKit::VerifyAccessToken(result.tokenID, result.permissionName));
        goalTokenId = result.tokenID;
    }

    uint32_t goalTokenId = 0;
};

/**
 * @tc.name: UserPolicyForDeleteTest
 * @tc.desc: Set the authorization status based on the user policy during Delete hap installation
 * @tc.type: FUNC
 * @tc.require:Issue Number
 */
HWTEST_F(EdmPolicySetTest, UserPolicyForDeleteTest, TestSize.Level1)
{
    /* Init test hap */
    g_testHapInfoParams.userID = MOCK_USER_ID_10001;
    AccessTokenIDEx fullIdUser1;
    ...
    /* Register internet permission listener */
    PermStateChangeScope scopeInfo;
    scopeInfo.permList = { INTERNET };
    scopeInfo.tokenIDs = {};
    auto callbackPtr = std::make_shared<CbCustomizeTest>(scopeInfo);
    int32_t ret = AccessTokenKit::RegisterPermStateChangeCallback(callbackPtr);
    EXPECT_EQ(RET_SUCCESS, ret);
    ...
    ret = AccessTokenKit::InitUserPolicy(userListBefore, permList);
    EXPECT_EQ(ret, 0);
    EXPECT_EQ(AccessTokenKit::VerifyAccessToken(fullIdUser1.tokenIdExStruct.tokenID, INTERNET), PERMISSION_GRANTED);
    EXPECT_EQ(AccessTokenKit::VerifyAccessToken(fullIdUser2.tokenIdExStruct.tokenID, INTERNET), PERMISSION_DENIED);
    EXPECT_EQ(AccessTokenKit::VerifyAccessToken(fullIdUser2.tokenIdExStruct.tokenID, INTERNET, true),
    ...
    EXPECT_EQ(callbackPtr->goalTokenId, fullIdUser1.tokenIdExStruct.tokenID);

    ret = AccessTokenKit::ClearUserPolicy();
    EXPECT_EQ(ret, 0);
    ret = AccessTokenKit::UnRegisterPermStateChangeCallback(callbackPtr);
