...
HWTEST_F(AccessTokenKitTest, GetUserGrantedPermissionUsedType001, TestSize.Level1)
{
    ...
    EXPECT_EQ(INVALID_USED_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(selfTokenId_, accessBluetooth));
    AccessTokenID tokenID = AllocTestToken(g_infoManagerTestInfoParms, g_infoManagerTestPolicyPrams);

    EXPECT_EQ(INVALID_USED_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(0, accessBluetooth));

    EXPECT_EQ(INVALID_USED_TYPE,
        AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, "ohos.permission.ACCELEROMETER"));

    EXPECT_EQ(INVALID_USED_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, "ohos.permission.xxxxx"));

    EXPECT_EQ(INVALID_USED_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, accessBluetooth));
}
...
HWTEST_F(AccessTokenKitTest, GetUserGrantedPermissionUsedType002, TestSize.Level1)
{
    ...
    EXPECT_EQ(SEC_COMPONENT_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, accessBluetooth));

    EXPECT_EQ(NORMAL_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, sendMessages));

    EXPECT_EQ(INVALID_USED_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, writeCalendar));
}
...
HWTEST_F(AccessTokenKitTest, GetUserGrantedPermissionUsedType003, TestSize.Level1)
{
    ...
    EXPECT_EQ(SEC_COMPONENT_TYPE, AccessTokenKit::GetUserGrantedPermissionUsedType(tokenID, distributedDatasync));
}
