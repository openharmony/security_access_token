...
{
    std::lock_guard<std::mutex> lock(startRemoteUsingPermInputMutex_);
    remoteCacheList_.emplace_back(info);
    LOGI(PRI_DOMAIN, PRI_TAG, "Cache is added.");
}

void PrivacyManagerClient::DeleteRemoteInputCache(const RemoteCallerInfo& info, const std::string& permissionName)
{
    std::lock_guard<std::mutex> lock(startRemoteUsingPermInputMutex_);
    for (auto it = remoteCacheList_.begin(); it != remoteCacheList_.end(); ++it) {
        if ((it->remoteDeviceId == info.remoteDeviceId) && (it->permissionName == permissionName)) {
            remoteCacheList_.erase(it);
            LOGI(PRI_DOMAIN, PRI_TAG, "Cache is cleared.");
            break;
        }
    }
...
int32_t PrivacyManagerClient::StartRemoteUsingPermission(
    const RemoteCallerInfo& info, const std::string& permissionName)
{
    auto proxy = GetProxy();
    if (proxy == nullptr) {
        LOGE(PRI_DOMAIN, PRI_TAG, "Proxy is null.");
    ...
    usedInfo.remoteDeviceId = info.remoteDeviceId;
    usedInfo.remoteDeviceName = info.remoteDeviceName;
...
int32_t PrivacyManagerClient::StopRemoteUsingPermission(const RemoteCallerInfo& info, const std::string& permissionName)
{
    DeleteRemoteInputCache(info, permissionName);
