...
namespace {
static constexpr OHOS::HiviewDFX::HiLogLabel LABEL = { LOG_CORE, 0xD005A08, "AniScreenLockFileManager" };
...
static ani_int AcquireAccessSync([[maybe_unused]] ani_env* env)
{
    ...
    ACCESSTOKEN_LOG_INFO(LABEL, "AcquireAccessSync begin.");
    if (env == nullptr) {
...
{
    int32_t flag = AccessStatus::ACCESS_DENIED;
    ACCESSTOKEN_LOG_INFO(LABEL, "AcquireAccessExcute begin.");
    if (env == nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Env null");
        return flag;
    }
    ...
    if (!CheckDataType(env, dataLockType)) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Invalid DataType.");
        return flag;
    }
...
static ani_int ReleaseAccessSync([[maybe_unused]] ani_env* env)
{
    ...
    ACCESSTOKEN_LOG_INFO(LABEL, "ReleaseAccessSync begin.");
    if (env == nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Env null");
        return flag;
    }
...
{
    int32_t flag = ReleaseStatus::RELEASE_DENIED;
    ACCESSTOKEN_LOG_INFO(LABEL, "ReleaseAccessExecute begin.");
    if (env == nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Env null");
        return flag;
    }
...
static ani_int QueryAppKeyStateSync([[maybe_unused]] ani_env* env)
{
    ...
    ACCESSTOKEN_LOG_INFO(LABEL, "QueryAppKeyStateSync begin.");
    if (env == nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Env null");
        return flag;
    }
...
static ani_int QueryAppKeyStateExecute([[maybe_unused]] ani_env* env, ani_int datatype)
{
    ...
    ACCESSTOKEN_LOG_INFO(LABEL, "QueryAppKeyStateExecute begin.");
    if (env == nullptr) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "Env null");
        return flag;
    }
...
ANI_EXPORT ani_status ANI_Constructor(ani_vm* vm, uint32_t* result)
{
    ...
    if (ANI_OK != env->FindNamespace("@ohos.ability.screenLockFileManager.screenLockFileManager", &ns)) {
        ACCESSTOKEN_LOG_ERROR(LABEL, "FindNamespace screenLockFileManager failed.");
        return ANI_NOT_FOUND;
    }
