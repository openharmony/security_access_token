...
static bool CheckManualSettingPerm(ani_env* env, const std::string& permissionName, ani_object& error)
{
    ...
    if (!GetPermissionBriefDef(permissionName, permissionBriefDef) || !IsSupportPermission(permissionName)) {
    ...
    [[maybe_unused]] ani_object object, ani_object aniContext, ani_string aniPermission, ani_object callback)
{
    ...
    (void)HiSysEventWrite(HiviewDFX::HiSysEvent::Domain::ACCESS_TOKEN, "REQUEST_PERMISSIONS_FROM_USER",
        ...
        "TOKENID", asyncContext->tokenId, "EXTRA_INFO", asyncContext->permissionName);
