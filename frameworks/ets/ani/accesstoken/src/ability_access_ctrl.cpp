...
static bool IsDynamicRequest(std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    for (const auto& permState : permList) {
        ACCESSTOKEN_LOG_INFO(
            LABEL, "Permission: %{public}s: state: %{public}d", permState.permissionName.c_str(), permState.state);
...
static ani_object ConvertAniArray(ani_env* env, std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    if (asyncContext->result != RET_SUCCESS) {
        ...
        BusinessErrorAni::ThrowParameterTypeError(env, stsCode, "ConvertAniArray", GetErrorMessage(stsCode));
        return nullptr;
    }
    return aObject;
}
...
static ani_object InitializeAndValidateAsyncContext(
    ani_env* env, ani_object aniContext, ani_object permission, std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    asyncContext = std::make_shared<RequestAsyncContext>();
    ...
    processArrayClass(env, nullptr, permission, asyncContext->permissionList);
    ...
    [[maybe_unused]] ani_env* env, [[maybe_unused]] ani_object object, ani_object aniContext, ani_object permission)
{
    ...
    ani_object result = InitializeAndValidateAsyncContext(env, aniContext, permission, asyncContext);
    if (result != nullptr) {
        return result;
    }
    ...
    asyncContext->loadlock.lock();
    auto g_objectContext = ConvertAniArray(env, asyncContext);
    asyncContext->loadlock.unlock();
    return g_objectContext;
}
