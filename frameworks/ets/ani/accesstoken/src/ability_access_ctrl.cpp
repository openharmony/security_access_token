...
static void UpdateGrantPermissionResultOnly(
    const std::vector<std::string>& permissions, const std::vector<int32_t>& grantResults,
    std::shared_ptr<RequestAsyncContext>& data, std::vector<int32_t>& newGrantResults)
{
    ...
    ani_int tokenID, ani_string permissionName)
{
    ...
    if (tokenID == 0) {
        BusinessErrorAni::ThrowParameterTypeError(
            env, STSErrorCode::STS_ERROR_PARAM_INVALID, "CheckAccessTokenSync", GetParamErrorMsg("tokenID", "int32_t"));
        return AccessToken::PermissionState::PERMISSION_DENIED;
    }
