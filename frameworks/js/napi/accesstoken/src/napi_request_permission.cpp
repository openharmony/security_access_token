...
static void GetInstanceId(std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    task();
#endif
    LOGI(AT_DOMAIN, AT_TAG, "Instance id: %{public}d, uiContentFlag: %{public}d",
    ...
    const std::shared_ptr<UIExtensionCallback>& uiExtCallback)
{
    auto task = [asyncContext, want, uiExtensionCallbacks, uiExtCallback]() {
        ...
        LOGI(AT_DOMAIN, AT_TAG,
            "Create end, sessionId: %{public}d, tokenId: %{public}d, permNum: %{public}zu",
            sessionId, asyncContext->tokenId, asyncContext->permissionList.size());
        if (sessionId == 0) {
            LOGE(AT_DOMAIN, AT_TAG, "Create component failed, sessionId is 0");
            asyncContext->result = RET_FAILED;
            asyncContext->uiExtensionFlag = false;
            return;
        }
    ...
    task();
#endif
    LOGI(AT_DOMAIN, AT_TAG, "Instance id: %{public}d", asyncContext->instanceId);
}
...
static void CloseModalUIExtensionMainThread(std::shared_ptr<RequestAsyncContext>& asyncContext, int32_t sessionId)
{
    auto task = [asyncContext, sessionId]() {
        ...
        LOGI(AT_DOMAIN, AT_TAG, "Close end, sessionId: %{public}d", sessionId);
    };
#ifdef EVENTHANDLER_ENABLE
    if (asyncContext->handler_ != nullptr) {
    ...
    const napi_env &env, const napi_value &value, std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    if (status != napi_ok || !stageMode) {
        ...
        if (context == nullptr) {
            LOGE(AT_DOMAIN, AT_TAG, "Get context failed");
            return nullptr;
        }
        ...
        if (asyncContext->abilityContext != nullptr) {
            ...
            LOGW(AT_DOMAIN, AT_TAG, "Convert to ability context failed");
            ...
            if (asyncContext->uiExtensionContext == nullptr) {
                LOGE(AT_DOMAIN, AT_TAG, "Convert to ui extension context failed");
                return nullptr;
            }
...
static void ResultCallbackJSThreadWorker(uv_work_t* work, int32_t status)
{
    ...
    if (retCB->data->result != RET_SUCCESS) {
        LOGE(AT_DOMAIN, AT_TAG, "Result is: %{public}d", retCB->data->result);
        result = RET_FAILED;
    }
    if (retCB->grantResults.empty()) {
        LOGE(AT_DOMAIN, AT_TAG, "GrantResults empty");
        result = RET_FAILED;
    }
...
void AuthorizationResult::WindowShownCallback()
{
    ...
    LOGD(AT_DOMAIN, AT_TAG, "OnRequestPermissionsFromUser async callback is called end");
}

static void CreateServiceExtension(std::shared_ptr<RequestAsyncContext> asyncContext)
{
    if (!asyncContext->uiAbilityFlag) {
        LOGE(AT_DOMAIN, AT_TAG, "UIExtension ability can not pop service ablility window!");
    ...
    if (remoteObject == nullptr) {
        LOGE(AT_DOMAIN, AT_TAG, "Create window failed!");
        asyncContext->needDynamicRequest = false;
        asyncContext->result = RET_FAILED;
        return;
    }
    ...
    LOGI(AT_DOMAIN, AT_TAG, "Request end, ret: %{public}d, tokenId: %{public}d, permNum: %{public}zu",
...
bool NapiRequestPermission::IsDynamicRequest(std::shared_ptr<RequestAsyncContext>& asyncContext)
{
    ...
    for (const auto& permState : permList) {
        LOGI(AT_DOMAIN, AT_TAG, "Permission: %{public}s: state: %{public}d",
    ...
    if (permList.size() != asyncContext->permissionList.size()) {
        LOGE(AT_DOMAIN, AT_TAG, "Returned permList size: %{public}zu.", permList.size());
        return false;
    }
...
{
    LOGI(AT_DOMAIN, AT_TAG, "ResultCode is %{public}d", resultCode);
...
{
    LOGD(AT_DOMAIN, AT_TAG, "RequestPermissionsFromUser begin.");
    ...
    LOGD(AT_DOMAIN, AT_TAG, "RequestPermissionsFromUser end.");
...
void NapiRequestPermission::RequestPermissionsFromUserExecute(napi_env env, void* data)
{
    ...
    if (asyncContextHandle->asyncContextPtr->tokenId != selfTokenID) {
        LOGE(AT_DOMAIN, AT_TAG, "The context tokenID: %{public}d, selfTokenID: %{public}d.",
    ...
    if (!IsDynamicRequest(asyncContextHandle->asyncContextPtr)) {
        LOGI(AT_DOMAIN, AT_TAG, "It does not need to request permission");
    ...
    if (asyncContextHandle->asyncContextPtr->info.grantBundleName == ORI_PERMISSION_MANAGER_BUNDLE_NAME) {
        LOGI(AT_DOMAIN, AT_TAG, "Pop service extension dialog, uiContentFlag=%{public}d",
        ...
        LOGI(AT_DOMAIN, AT_TAG, "Pop service extension dialog, instanceId is -1.");
        CreateServiceExtension(asyncContextHandle->asyncContextPtr);
        ...
        LOGI(AT_DOMAIN, AT_TAG, "Pop ui extension dialog");
        ...
        if (!asyncContextHandle->asyncContextPtr->uiExtensionFlag) {
            LOGW(AT_DOMAIN, AT_TAG,
                "Pop uiextension dialog fail, start to pop service extension dialog.");
            RequestAsyncInstanceControl::AddCallbackByInstanceId(asyncContextHandle->asyncContextPtr);
        }
    }
}
...
{
    LOGD(AT_DOMAIN, AT_TAG, "GetPermissionsStatus begin.");
    ...
    LOGD(AT_DOMAIN, AT_TAG, "GetPermissionsStatus end.");
    context.release();
    return result;
}
    ...
    RequestAsyncContext& asyncContext)
{
    ...
    LOGD(AT_DOMAIN, AT_TAG,
        "TokenID = %{public}d, permissionList size = %{public}zu", asyncContext.tokenId,
...
void NapiRequestPermission::GetPermissionsStatusExecute(napi_env env, void *data)
{
    ...
    std::vector<PermissionListState> permList;
    for (const auto& permission : asyncContext->permissionList) {
        LOGD(AT_DOMAIN, AT_TAG, "Permission: %{public}s.", permission.c_str());
        PermissionListState permState;
        permState.permissionName = permission;
        permState.state = INVALID_OPER;
        permList.emplace_back(permState);
    ...
    LOGD(AT_DOMAIN, AT_TAG,
        "PermList size: %{public}zu, asyncContext->permissionList size: %{public}zu.",
    ...
    for (const auto& permState : permList) {
        LOGD(AT_DOMAIN, AT_TAG, "Permission: %{public}s", permState.permissionName.c_str());
...
void NapiRequestPermission::GetPermissionsStatusComplete(napi_env env, napi_status status, void *data)
{
    ...
    if ((asyncContext->permissionQueryResults.empty()) && asyncContext->result == JsErrorCode::JS_OK) {
        LOGE(AT_DOMAIN, AT_TAG, "PermissionQueryResults empty");
        asyncContext->result = RET_FAILED;
    }
    napi_value result;
    ...
    std::shared_ptr<RequestAsyncContext>& asyncContext, bool& isDynamic)
{
    ...
    if (!NapiRequestPermission::IsDynamicRequest(asyncContext)) {
        LOGI(AT_DOMAIN, AT_TAG, "It does not need to request permission exsion");
...
{
    LOGI(AT_DOMAIN, AT_TAG, "InstanceId: %{public}d", asyncContext->instanceId);
    {
        ...
        if (iter != instanceIdMap_.end()) {
            LOGI(AT_DOMAIN, AT_TAG, "InstanceId: %{public}d has existed.", asyncContext->instanceId);
...
void RequestAsyncInstanceControl::ExecCallback(int32_t id)
{
    ...
    bool isDynamic = false;
    {
        ...
        if (iter == instanceIdMap_.end()) {
            LOGI(AT_DOMAIN, AT_TAG, "Id: %{public}d not existed.", id);
            return;
        }
        while (!iter->second.empty()) {
            LOGI(AT_DOMAIN, AT_TAG,
                "Id: %{public}d, map size: %{public}zu.", id, iter->second.size());
        ...
        if (iter->second.empty()) {
            LOGI(AT_DOMAIN, AT_TAG, "Id: %{public}d, map is empty", id);
